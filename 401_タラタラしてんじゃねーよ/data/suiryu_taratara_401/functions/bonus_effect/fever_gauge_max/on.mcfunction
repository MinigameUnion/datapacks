#> suiryu_taratara_401:bonus_effect/fever_gauge_max/on
#
# FEVERゲージMAX
# ON
#
# @within function suiryu_taratara_401:bonus_effect/fever_gauge_max/off
...
# 所持ミニが消費ミニより多ければON
  ...
  ## 看板書き換え
    execute if score @s coin_000 >= #miniCount_401 suiryu_taratara_401.trigger run data modify block 3991 51 3 Text1 set value '{"text":"FEVERゲージMAX","color":"light_purple","clickEvent":{"action":"run_command","value":"/execute unless score #PremiumTicketUsing suiryu_taratara_401.trigger matches 1 run function suiryu_taratara_401:bonus_effect/fever_gauge_max/off"}}'
    execute if score @s coin_000 >= #miniCount_401 suiryu_taratara_401.trigger run data modify block 3991 51 3 Text4 set value '{"text":"ON","color":"yellow"}'

  ## スコアセット
    execute if score @s coin_000 >= #miniCount_401 suiryu_taratara_401.trigger run scoreboard players set #BonusEffect.FeverGaugeMax suiryu_taratara_401.trigger 1
