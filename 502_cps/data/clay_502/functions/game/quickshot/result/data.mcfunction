...
tellraw @s[tag=winner_502] ["",{"text":"・勝利ボーナス : ","color":"gray"},{"score":{"objective": "num_502","name": "#WIN_BONUS"},"color":"white"},{"text":"ミニ","color":"white"}]
scoreboard players operation @s[tag=winner_502] point_502 += #WIN_BONUS num_502

execute if score #TOTAL num_502 >= #PERFECT num_502 run tellraw @s ["",{"text":"・パーフェクト : ","color":"gray"},{"score":{"objective": "num_502","name": "#PERFECT_BONUS"},"color":"white"},{"text":"ミニ","color":"white"}]
execute if score #TOTAL num_502 >= #PERFECT num_502 run scoreboard players operation @s point_502 += #PERFECT_BONUS num_502
