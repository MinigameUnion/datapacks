...
execute as @a[scores={playing_000=502},tag=creating_502,limit=1] run function clay_502:game/default/player

tag @e[dx=47,dy=31,dz=31,tag=creating_502] remove creating_502

data modify entity @s ArmorItems[0].tag.match.tag.type set value 1b
data modify entity @s ArmorItems[0].tag.match.tag.join set value 0b
data modify entity @s ArmorItems[0].tag.match.tag.spectate set value 1b
data modify entity @s ArmorItems[0].tag.match.tag.display.Name set value '{"text":"スタンダード"}'
data modify entity @s ArmorItems[0].tag.match.tag.display.Lore set value ['ス','タ','ン','ダ','ー','ド']
