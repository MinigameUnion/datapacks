...
# ゲーム開始
execute as 0-0-321-0-0 if entity @s[tag=Feeded_801] run tellraw @a[tag=Feeder_801] ["",{"translate":"%1$s %2$s %1$s\nMineSweeperの羊バージョンです。ハズレの羊を避けてすべての羊の毛を刈るとクリアです。複数人でプレイできます！\n%3$s","with":[{"text":".....","color":"gray","obfuscated":true},{"text":"Mine Sheeper","underlined":true,"bold":true},{"text":"[取得ミニについて]","color":"green","hoverEvent":{"action":"show_text","value":{"text":"羊の毛を刈ると羊毛が入手できます。複数マスが連鎖で開いた場合は開いたマスの分手に入ります。\n最終的に持っていた羊毛の個数にボーナスを考慮したミニが得られます。\nボーナスの例：ゲームクリア、難易度、タイム、旗不使用"}}}]}]
