...
execute if score @s InfoTrigger_801 matches 2 run tellraw @s ["",{"translate":"[すごいハサミについて] すごいハサミではハズレの羊の毛を刈っても%1$s！(かわりに目印をつけます。)ゲーム毎に一定回数まで無料で、それ以降はミニを消費することで使用できます。無料使用回数を残した場合は最終的なミニにボーナスが得られます。","with":[{"text":"ゲームオーバーになりません","underlined":true}]}]
...
execute if score @s InfoTrigger_801 matches 4 run tellraw @s ["",{"translate":"[うまく動かないときは] ・アイテムがなくなった→再ログインしてみてください。\n・エンティティが出ない→下ステージのロゴ反対側の看板から退出/初期化してみてください。","with":[]}]
