...
#イジケ
    #スコアの設定
    scoreboard players set @e[tag=Ghost_base_102,type=item_display,scores={_State_102=2}] _State_102 3
    scoreboard players set #izike_102 _Point_102 100
    #ゴーストの視点切り替え
    execute as @e[tag=Ghost_base_102,type=item_display] run function pac_man_102:mobs/ghost/turn/back
...
scoreboard players remove @e[tag=Ghost_base_102,type=item_display,scores={_State_102=1..}] _Count_102 1

#サウンド
execute as @a[scores={playing_000=102}] at @s run playsound minecraft:block.note_block.didgeridoo voice @s ~ ~ ~ 1 2

#エサの削除
kill @e[tag=PowerPellet_102,type=item_display,dx=0,dy=0,dz=0,limit=1]