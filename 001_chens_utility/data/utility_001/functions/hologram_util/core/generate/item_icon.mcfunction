...
#ベースsummon
    summon armor_stand ~ 300 ~ {Marker:1b,Invisible:1b,Tags:[hologram_001,hologram_stand_001,summonNow_001],Passengers:[{id:"item",Item:{id:"barrier",Count:1b,tag:{CustomModelData:0}},NoGravity:1b,PickupDelay:32767s,Tags:[hologram_001,hologram_item_001,summonNow_001]}]}
#HologramID
    execute store result score @e[tag=hologram_item_001,tag=summonNow_001,distance=..0,y=300,limit=1] hologram_id_001 run data get storage utility_001:hologram_util TextGroup[-1].HologramID
#えんちゃ
...
#Tempを適用
    data modify entity @e[type=item,tag=hologram_item_001,tag=summonNow_001,distance=..0,y=300,limit=1] Item set from storage utility_001:hologram_util Temp
#AnyNBT
    execute if data storage utility_001:hologram_util TextGroup[-1].ItemIcon[-1].Setting.NBT run data modify entity @e[type=item,tag=hologram_item_001,tag=summonNow_001,distance=..0,y=300,limit=1] {} merge from storage utility_001:hologram_util TextGroup[-1].ItemIcon[-1].Setting.NBT
#Offset.y
    ...
    execute if score #Move utility_001 matches 1.. as @e[type=armor_stand,tag=hologram_stand_001,tag=summonNow_001,distance=..0.001,limit=1] rotated ~ 90 run function utility_001:hologram_util/core/generate/move/31
#TagDelete
    tag @e[tag=hologram_001,tag=summonNow_001] remove summonNow_001
#remove
